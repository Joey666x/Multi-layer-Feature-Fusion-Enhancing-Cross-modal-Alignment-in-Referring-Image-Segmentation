...
class Manager(nn.Module):
    ...
    def forward(self, hidden_states, cross_modal_hidden_states, masks, extra_query=None, is_training=False):
        return self.manager_layer(hidden_states, cross_modal_hidden_states, masks, extra_query=extra_query,
...
class SAUE(nn.Module):
    def __init__(self, config, routed_layers, layer_index=0):
        ...
        if self.config['manager_weight_type'] == 'scalar':
            ...
            self.layer_scores = nn.Parameter(self.layer_scores.repeat(1, 1, 1, self.config['hidden_size']),
                                             requires_grad=self.config['manager_learnable'])

        # normalization for reps
        self.LayerNorm = nn.ModuleList(
            [nn.LayerNorm(self.config['hidden_size']) for _ in range(routed_layers + num_previous)])

        # learnable temperature
        self.softmax_temperature = nn.Parameter(torch.ones(1) * self.config['manager_softmax_temperature'],
                                                requires_grad=self.config['manager_softmax_temperature_learnable'])
        # self.softmax_temperature_cross = nn.Parameter(torch.ones(1) * self.config['manager_softmax_temperature'],
        #                                               requires_grad=self.config[
        #                                                   'manager_softmax_temperature_learnable'])

    def aggregate_reps(self, layer_scores, reps, masks, is_training=False):
        # layer_scores: 1 x N x 1 x (1 / D)
        # reps: B x N x L x D
        if is_training:
            layer_scores = layer_scores.expand_as(reps) + torch.cat((torch.normal(mean=0, std=1 / (self.routed_layers),
                                                                                  size=reps[:,
                                                                                       :self.routed_layers].shape,
                                                                                  device=layer_scores.device),
                                                                     torch.zeros_like(reps[:, self.routed_layers:])),
                                                                    dim=1)
    ...
    def forward(self, hidden_states, cross_modal_hidden_states, masks, extra_query=None, is_training=False):
        hidden_states = torch.stack([self.LayerNorm[i](hidden_states[:, i]) for i in range(hidden_states.shape[1])],
                                    dim=1)
        ...
        if self.layer_index == 0:
            ...
            cross_modal_hidden_states = torch.stack(
                [self.LayerNorm[i + hidden_states.shape[1]](cross_modal_hidden_states[:, i]) for i in
                 range(cross_modal_hidden_states.shape[1])], dim=1)
            hidden_states = self.aggregate_reps(layer_scores_,
                                                torch.cat((hidden_states, cross_modal_hidden_states), dim=1), masks,
                                                is_training=is_training)
...
class AAUE(nn.Module):
    def __init__(self, config, routed_layers, layer_index=0):
        ...
        if self.config['manager_weight_type'] == 'scalar':
            self.layer_scores_cross = nn.Parameter(self.layer_scores_cross,
                                                   requires_grad=self.config['manager_learnable'])
        elif self.config['manager_weight_type'] == 'vector':
            self.layer_scores_cross = nn.Parameter(self.layer_scores_cross.repeat(1, 1, 1, self.config['hidden_size']),
                                                   requires_grad=self.config['manager_learnable'])
    ...
    def forward(self, hidden_states, cross_modal_hidden_states, masks, extra_query=None, is_training=False):
        ...
        fused_query = torch.cat((torch.softmax(self.fusion_attention(cross_modal_hidden_states[:, -1], extra_query),
                                               dim=-1) @ extra_query, cross_modal_hidden_states[:, -1]),
                                dim=-1)  # B x L x 2D
        ...
        layer_scores_generate = torch.cat((layer_scores_generate,
                                           self.layer_scores_cross.expand(layer_scores_generate.shape[0], -1,
                                                                          layer_scores_generate.shape[2], -1)), dim=1)
